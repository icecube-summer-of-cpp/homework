...
class fin_diff{
    ...
    public:
        fin_diff(const F& f, const T& h): f_(f), h_(h){} 
...
template<typename F, typename T>
double trapezoid(const F& f, T a, T b, int N=1000) {
    // I = h/2*f(a) + h/2*f(b) + h sum_{j=1}^{n-1} f(a + jh)
    double h = (b-a)/N;
...
int main (int argc, char **argv) 
{
    ...
    std::cout << "int_low^up dx df/dx = "<< trapezoid(fin_diff<decltype(exp3f), double>(exp3f, h_fin_diff), lower_bound, upper_bound) << std::endl;
    std::cout << "int_low^up dx df/dx = "<< trapezoid(fin_diff<decltype(exp3f), double>(exp3f, h_fin_diff), lower_bound, upper_bound, 10000) << std::endl;
    std::cout << "int_low^up dx df/dx = "<< trapezoid(fin_diff<decltype(exp3f), double>(exp3f, 0.00001), lower_bound, upper_bound) << std::endl;
    std::cout << "int_low^up dx df/dx = "<< trapezoid(fin_diff<decltype(exp3f), double>(exp3f, 0.00001), lower_bound, upper_bound, 10000) << std::endl;
    /*
     * int_low^up df/dx dx =  f |_low^up = f(up) - f(low)
     */
    std::cout << "f(up)-f(low) = " << exp3f(upper_bound) - exp3f(lower_bound) << std::endl;
